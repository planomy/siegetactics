...
export function repairTrainingGate(gate) {
  ...
  for (const topic of getGateTopics()) {
    ...
    if (topicsDone.length >= GATE.requiredTopics) continue;
...
export function isGateOpen(gate) {
  ...
  return (
    g.timesTablesDone.length >= GATE.requiredTables &&
    g.topicsDone.length >= GATE.requiredTopics
  );
}
...
export function gateProgress(gate) {
  ...
  const topics = Math.min(g.topicsDone.length, GATE.requiredTopics);
  const total = GATE.requiredTables + GATE.requiredTopics;
...
export function recordTimesTable(gate, table, accuracy) {
  ...
  if (gate.timesTablesDone.length >= GATE.requiredTables) {
    return { added: false, gate, reason: 'All table drills done — train another topic!' };
  }
  return {
    added: true,
...
export function recordTopicUnit(gate, topicId, unitId) {
  ...
  if (unitAlreadyDone && topicAlreadyDone) {
    return { added: false, gate: g, reason: 'You already trained that unit this cycle.' };
  }

  if (unitAlreadyDone && !topicAlreadyDone) {
    if (g.topicsDone.length >= GATE.requiredTopics) {
      return { added: false, gate: g, reason: 'Topic drills full — try a new times table!' };
    }
    return {
      added: true,
